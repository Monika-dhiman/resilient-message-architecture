...
export class RabbitmqConnectionService {
  ...
  async deadLetter(message: amqp.Message, error: Error) {
  ...
  private convertToPoisonMessageFormat(message: amqp.Message, error: Error) {
  ...
  async retry(message: amqp.Message, error: Error) {
  ...
  private incrementRedeliveryCountAndSetTTL(message: amqp.Message) {
    ...
    message.properties.headers['retry_owner'] = this.config.appName;
  ...
  robustParseMessageContent(message: amqp.Message) {
    try {
  ...
  private repairMessageContent(message: amqp.Message) {
    try {
